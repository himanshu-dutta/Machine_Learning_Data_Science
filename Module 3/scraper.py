...
def get_live_data(symbol,interval=3):
    link = 'https://www.finance.yahoo.com/quote/{}?p={}&.tsrc=fin-srch'.format(symbol,symbol)
    output = []
    try:
        start = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print('Live Stocks for: ', symbol)
        print('%20s  |%20s  |%20s  |%20s  |%20s  |' % ('Date','Current Value', 'Change','% Change', 'Market Status'))
        ...
        while(True):
            ...
            if 'open' in current[2]:
                rec.append('Open')
            else: rec.append('Close')
            output.append(rec)
            print('%20s  |%20.3f  |%20.3f  |%20.3f  |%20s  |' % (rec[0],rec[1],rec[2],rec[3],rec[4]))
            time.sleep(interval)
        ...
        print('\nSaved current output as: ',symbol + '__' + start + '__' + end+'.csv')
def main():
